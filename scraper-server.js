...
async function initDatabase() {
    try {
        ...
        // Unique sellers table
        await pool.query(`
            CREATE TABLE IF NOT EXISTS unique_sellers (
                id SERIAL PRIMARY KEY,
                seller_id VARCHAR(255) UNIQUE NOT NULL,
                is_dazmall BOOLEAN DEFAULT FALSE,
...
// Scraper function with progress updates
async function scrapeDarazWithProgress(cookieString, sessionId, sellersOnly = false) {
    ...
    let pageNo = 1;
    let streamId = null;
    const maxPages = sellersOnly ? 999 : 20; // Unlimited for sellers mode!

    console.log(`🚀 Starting scrape - Mode: ${sellersOnly ? 'SELLERS (UNLIMITED)' : 'PRODUCTS (20 pages)'}`);
    ...
    // First page
    scrapingProgress[sessionId] = {
        status: 'scraping',
        page: 1,
        totalPages: sellersOnly ? '∞' : maxPages,
        productsScraped: 0,
        mode: sellersOnly ? 'sellers' : 'products',
    ...
    scrapingProgress[sessionId] = {
        ...
        page: 1,
        totalPages: sellersOnly ? '∞' : maxPages,
        productsScraped: allItems.length,
        mode: sellersOnly ? 'sellers' : 'products',
        message: `Page 1 complete - ${allItems.length} products scraped`
    };
    ...
    if (endPage) {
        ...
        if (streamId) {
            ...
            while (pageNo < maxPages) {
                ...
                if (items.length === 0) {
                    console.log('⚠️  No more items - DONE!');
                    break;
                }
                ...
                scrapingProgress[sessionId] = {
                    ...
                    productsScraped: allItems.length,
                    mode: sellersOnly ? 'sellers' : 'products',
                    message: `Page ${pageNo} complete - ${allItems.length} total products`
                };
...
// Scraper function (keep old one for compatibility)
...
// Store active scraping progress
let scrapingProgress = {};

// API: Get scraping progress (Server-Sent Events)
app.get('/api/scrape-progress/:sessionId', (req, res) => {
    ...
    const interval = setInterval(() => {
        if (scrapingProgress[sessionId]) {
            res.write(`data: ${JSON.stringify(scrapingProgress[sessionId])}\n\n`);
            
            // If complete or error, clean up
            if (scrapingProgress[sessionId].status === 'complete' || 
                scrapingProgress[sessionId].status === 'error') {
                setTimeout(() => {
                    delete scrapingProgress[sessionId];
...
// API: Run sellers-only scraper (UNLIMITED PAGES)
app.post('/api/scrape-sellers', async (req, res) => {
    ...
    scrapingProgress[sessionId] = {
        ...
        productsScraped: 0,
        mode: 'sellers',
        message: 'Initializing sellers scraper (UNLIMITED)...'
    };

    // Send session ID immediately
    res.json({ sessionId, message: 'Sellers scraping started (unlimited pages)' });

    // Run scraper in background
    (async () => {
        try {
            console.log('🚀 Starting SELLERS-ONLY scrape (UNLIMITED PAGES)...');
            scrapingProgress[sessionId] = {
                status: 'scraping',
                page: 0,
                totalPages: '∞',
                productsScraped: 0,
                mode: 'sellers',
                message: 'Starting unlimited sellers scraping...'
            };

            // Run scraper in SELLERS MODE (unlimited pages)
            const result = await scrapeDarazWithProgress(cookies, sessionId, true);
            
            scrapingProgress[sessionId] = {
                status: 'processing',
                page: result.pages_scraped,
                totalPages: '∞',
                productsScraped: result.items.length,
                mode: 'sellers',
                message: 'Extracting and updating sellers...'
            };
            ...
            for (const item of result.items) {
                const sellerId = item.seller?.sellerId || item.sellerId;
                if (sellerId) {
                    const isDazmall = (item.buType || []).includes('DazMall');
                    uniqueSellers.set(sellerId, {
                        seller_id: sellerId,
                        is_dazmall: isDazmall
                    });
                }
            }

            // Update sellers database
            let newSellers = 0;
            let updatedSellers = 0;

            for (const [sellerId, seller] of uniqueSellers) {
                const result = await pool.query(`
                    INSERT INTO unique_sellers (seller_id, is_dazmall, last_seen, appearance_count)
                    VALUES ($1, $2, CURRENT_TIMESTAMP, 1)
                    ON CONFLICT (seller_id) 
                    DO UPDATE SET 
                        last_seen = CURRENT_TIMESTAMP,
                        appearance_count = unique_sellers.appearance_count + 1,
                        is_dazmall = EXCLUDED.is_dazmall OR unique_sellers.is_dazmall
                    RETURNING (xmax = 0) AS inserted
                `, [sellerId, seller.is_dazmall]);
            ...
            console.log(`✅ Processed ${uniqueSellers.size} unique sellers (${newSellers} new, ${updatedSellers} updated)`);
            ...
            scrapingProgress[sessionId] = {
                ...
                updatedSellers: updatedSellers,
                timeTaken: result.time_taken,
                mode: 'sellers',
                message: 'Sellers scraping complete!'
            };

        } catch (error) {
            console.error('Sellers scrape error:', error);
            scrapingProgress[sessionId] = {
                status: 'error',
    ...
    (async () => {
        try {
            ...
            scrapingProgress[sessionId] = {
                ...
                totalPages: 20,
                productsScraped: 0,
                message: 'Starting to scrape Daraz...'
            };

            // Modified scraper with progress updates
            const result = await scrapeDarazWithProgress(cookies, sessionId);
            ...
            // Update unique sellers
            scrapingProgress[sessionId].message = 'Updating sellers database...';
            const uniqueSellers = new Map();
            ...
            console.log(`✅ Saved ${products.length} products and updated ${uniqueSellers.size} sellers`);
            ...
            scrapingProgress[sessionId] = {
                ...
                totalPages: 20,
                productsScraped: result.items.length,
                sellersUpdated: uniqueSellers.size,
                timeTaken: result.time_taken,
                runId: runId,
...
// API: Get product hunting analytics
app.get('/api/product-hunting', async (req, res) => {
    try {
        // Hot products (appearing most frequently)
        const hotProducts = await pool.query(`
            SELECT 
                ...
                SUM(NULLIF(REGEXP_REPLACE(total_sales, '[^0-9]', '', 'g'), '')::INTEGER) as total_sales_sum,
                MAX(discount_price) as latest_price,
                MAX(scrape_date) as last_seen
            FROM flash_sale_products
            WHERE item_id IS NOT NULL AND item_id != ''
            GROUP BY item_id, title
            HAVING COUNT(*) > 1
            ORDER BY appearance_count DESC, total_sales_sum DESC
            LIMIT 20
        `);

        // Top selling products (by total sales across appearances)
        ...
        const trendingCategories = await pool.query(`
            SELECT 
                ...
                SUM(NULLIF(REGEXP_REPLACE(total_sales, '[^0-9]', '', 'g'), '')::INTEGER) as total_sales
            FROM flash_sale_products
        ...
        // DazMall vs Regular performance
        const storePerformance = await pool.query(`
            SELECT 
                is_dazmall,
                COUNT(*) as product_count,
                AVG(NULLIF(REGEXP_REPLACE(total_sales, '[^0-9]', '', 'g'), '')::INTEGER) as avg_sales
            FROM flash_sale_products
            WHERE total_sales IS NOT NULL AND total_sales != ''
            GROUP BY is_dazmall
        `);
        ...
        res.json({
            hot_products: hotProducts.rows,
            top_sellers: topSellers.rows,
            trending_categories: trendingCategories.rows,
            store_performance: storePerformance.rows
        });
    } catch (error) {
        console.error('Product hunting analytics error:', error);
        res.status(500).json({ error: error.message });
    }
});

// API: Download sellers master CSV
app.get('/api/download/sellers', async (req, res) => {
    try {
        ...
        const csv = generateCSV(sellers.rows);
        res.setHeader('Content-Type', 'text/csv');
        res.setHeader('Content-Disposition', 'attachment; filename="sellers_master.csv"');
        res.send(csv);
    } catch (error) {
        console.error('Sellers download error:', error);
        res.status(500).send('Error generating sellers CSV');
    }
});
